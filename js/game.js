...
//游戏主类
function game(canvas,cobj,runs,jumps,hinderImg,runA,zidanA,jumpA,dieA,life,score,zidanImg){
    this.canvas=canvas;
    this.cobj=cobj;
    this.hinderImg=hinderImg;
        ...
        for(var i=0;i<that.hinderArr.length;i++){
            ...
            if(hitPix(that.canvas,that.cobj,that.person,that.hinderArr[i])){
                if(!that.hinderArr[i].flag){
                    that.person.life--;
                    $(".life>span").html(that.person.life);
                    that.dieA.play();
